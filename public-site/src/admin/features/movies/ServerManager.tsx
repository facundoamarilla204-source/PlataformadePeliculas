...
import { Tv, Plus, Trash2, GripVertical, CheckCircle2, XCircle, Loader2 } from 'lucide-react';
...
export const ServerManager: React.FC<ServerManagerProps> = ({ servers, onChange, movieData }) => {
  const [verifying, setVerifying] = useState<number | null>(null);
  const [verifyStatus, setVerifyStatus] = useState<Record<number, { status: 'success' | 'error', message: string }>>({});
  ...
  const verifyServer = async (index: number) => {
    const server = servers[index];
    setVerifying(index);
    try {
      const response = await api.post('/streaming/check-external', {
        provider: server.provider,
        tmdb_id: movieData?.tmdb_id || '',
        imdb_id: movieData?.imdb_id || '',
        type: movieData?.type || 'movie',
        season: movieData?.season || 0,
        episode: movieData?.episode || 0,
        config: server.config
      });
      const data = response.data;
      if (data.available || data.success) {
        setVerifyStatus(prev => ({ ...prev, [index]: { status: 'success', message: 'Servidor verificado correctamente' } }));
      } else {
        setVerifyStatus(prev => ({ ...prev, [index]: { status: 'error', message: data.error || data.message || 'El servidor no está disponible' } }));
      }
    } catch (err: any) {
      setVerifyStatus(prev => ({ ...prev, [index]: { status: 'error', message: err.message || 'Error de conexión' } }));
    } finally {
      setVerifying(null);
    }
  };
      ...
      case 'streamtape':
        return (
          <div className="space-y-1">
            ...
            <input
              type="text"
              value={server.config.fileCode || server.config.file_code || ''}
              onChange={(e) => updateConfig(index, 'file_code', e.target.value)}
              className="w-full px-2 py-1 bg-bg-tertiary border border-gray-700 rounded text-sm text-white"
    ...
    <div className="border-t border-gray-800 pt-6 mt-6">
      ...
      <div className="space-y-3">
        {servers.map((server, index) => (
          <div key={index} className="flex flex-col gap-4 p-4 border border-gray-700 rounded-lg bg-bg-secondary transition-all hover:border-gray-600">
            ...
            <div className="flex items-center gap-3 pt-3 border-t border-gray-800">
              <button
                type="button"
                onClick={() => verifyServer(index)}
                disabled={verifying === index}
                className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg bg-gray-800 hover:bg-gray-700 text-white transition-colors disabled:opacity-50"
              >
                {verifying === index ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Tv className="w-3.5 h-3.5" />}
                {verifying === index ? 'Verificando...' : 'Verificar Servidor'}
              </button>
              
              {verifyStatus[index] && (
                <div className={`flex items-center gap-1.5 text-xs ${verifyStatus[index].status === 'success' ? 'text-green-400' : 'text-red-400'}`}>
                  {verifyStatus[index].status === 'success' ? <CheckCircle2 className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
                  {verifyStatus[index].message}
                </div>
              )}
            </div>
          </div>
