...
import { LayoutDashboard, Film, FolderOpen, Image as ImageIcon, Settings, LogOut, Menu, X, Tv, Globe } from 'lucide-react';
...
export const Sidebar = () => {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/admi/panel', icon: LayoutDashboard },
    { name: 'Películas', path: '/admi/movies', icon: Film },
    { name: 'Categorías', path: '/admi/categories', icon: FolderOpen },
    { name: 'Banners Home', path: '/admi/banners', icon: ImageIcon },
    { name: 'Streaming', path: '/admi/streaming', icon: Tv },
    { name: 'Configuración', path: '/admi/settings', icon: Settings },
  ];
  ...
  return (
    <>
      ...
      <div className={`
        ...
        w-64 shrink-0 bg-bg-secondary border-r border-gray-800 h-screen flex flex-col
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-800 space-y-2">
          <Link
            href="/"
            target="_blank"
            className="flex items-center gap-3 px-4 py-3 w-full text-left rounded-xl text-text-secondary hover:bg-bg-tertiary hover:text-white transition-colors"
          >
